...
public:
    ...
    CubeMap(std::string HDRFilePath, float downSampleSize = 1.0)
    {
        ...
        Shader equirectangularToCubemapShader("equirectangularToCubemap", GLSL_VERTEX | GLSL_FRAGMENT);
...
private:
    ...
    void initDrawConfig(bool isHDR)
    {
        ...
        if (isHDR)
        {
            cubeMapShader = new Shader("cubemapHDR", GLSL_VERTEX | GLSL_FRAGMENT);
            cubeMapShader->use();
